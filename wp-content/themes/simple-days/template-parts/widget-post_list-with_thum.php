  ...
  if(get_query_var( 'popular_post_style')){
    ...
    if(!has_post_thumbnail()) {
      ...
      if(isset($matches [1] [0])){
        ...
        echo '<'.( (is_amp()) ?'amp-img layout="responsive"':'img').' src="'. esc_url(get_theme_mod( 'simple_days_no_img' , get_template_directory_uri() .'/assets/images/no_image.png')) .'" width="640" height="480" alt="No image available" />';
      }
    }else{
      //the_post_thumbnail(array(640, 480));
      $thumurl = wp_get_attachment_image_src( get_post_thumbnail_id() , array(45,45) );
      echo '<'.( (is_amp() )?'amp-img layout="responsive"':'img').' src="'.esc_url($thumurl[0]).'" width="'.esc_attr($thumurl[1]).'" height="'.esc_attr($thumurl[2]).'" alt="'.esc_html(get_the_title()).'" title="'.esc_html(get_the_title()).'" />';
    }
