...
class XH_Social_Hooks{
    ...
    public static function xunhuweb_cron(){
        ...
        if(is_admin()){
            ...
            try {
                XH_Social_Install::instance()->add_ons_update();
            } catch (Exception $e) {
       ...
       <table class="form-table">
			...
			<?php if($channels){
			    foreach ($channels as $channel){
			        ...
			        <tr >
						<th>
							<label><span class="xh-text"><a title="javascritp:void(0);" class="xh-social-item <?php echo $channel->svg?>"></a> <?php echo $channel->title?></span></label>
						</th>
						<td>
							<p>
...
//     }
    ...
    public static function wp_print_footer_scripts(){
        ...
        if(!$xunhuweb_cron){
            $xunhuweb_cron=true;
            ?><script type="text/javascript">if(jQuery){jQuery(function($){$.ajax({url: '<?php echo XH_Social::instance()->ajax_url('xunhuweb_cron',false,false)?>',type: 'post',timeout: 60 * 1000,async: true,cache: false});});}</script><?php
        }
    }
    ...
    public static function pre_get_avatar_data( $args, $id_or_email ){     
        ...
        if ( is_numeric( $id_or_email ) ) {
            ...
            if ( ! empty( $id_or_email->comment_type ) && ! in_array( $id_or_email->comment_type, (array) $allowed_comment_types ) ) {
                ...
                $url = apply_filters( 'get_avatar_url', false, $id_or_email, $args );
                $disable_replace = (defined('BP_AVATAR_DEFAULT')&&BP_AVATAR_DEFAULT==$url)
                                ||(defined('BP_AVATAR_DEFAULT_THUMB')&&BP_AVATAR_DEFAULT_THUMB==$url);
                
                if(apply_filters('wsocial_enable_replace_user_avatar', !$disable_replace,$user_ID,$url)){
                    $args['url'] =$url;
                }
