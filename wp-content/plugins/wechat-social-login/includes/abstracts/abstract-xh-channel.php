...
abstract class Abstract_XH_Social_Settings_Channel extends Abstract_XH_Social_Settings{   
    ...
    public function wp_insert_user_Info($ext_user_id,$userdata){
        ...
        try {
            ...
            //added 1.2.6
            do_action('wsocial_wp_insert_user',$wp_user_id,$ext_user_id,$userdata);
        } catch (Exception $e) {
    ...
    public function bindinfo($wp_user_id){
        ...
        if($this->get_ext_user_info_by_wp($wp_user_id)){
            ?>
            <a href="<?php echo XH_Social::instance()->channel->get_do_unbind_uri($this->id,XH_Social_Helper_Uri::get_location_uri());?>" class="xh-btn xh-btn-warning xh-btn-sm"><?php echo __('Unbind',XH_SOCIAL)?></a><?php 
        }else{
            ?>
            <a href="<?php echo XH_Social::instance()->channel->get_do_bind_redirect_uri($this->id,XH_Social_Helper_Uri::get_location_uri())?>" class="xh-btn xh-btn-primary xh-btn-sm"><?php echo __('Bind',XH_SOCIAL)?></a>
            <?php 
        }
