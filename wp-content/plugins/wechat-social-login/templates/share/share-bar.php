...
<script type="text/javascript">
	if(typeof window.xh_social_share!='function'){
		window.xh_social_share=function(settings){
			...
			var url = encodeURIComponent('<?php echo esc_url(apply_filters('wsocial_share_url', XH_Social_Helper_Uri::get_location_uri()))?>');
...
<div class="xh-social" style="clear:both;">
   <?php 
    foreach ($channel_share_enableds as $channel_id){
        ...
        if(!$channel){continue;}
        ?>
        <a href="javascript:void(0);" onclick="window.xh_social_share(<?php echo esc_attr(json_encode($channel->get_share_link()));?>);" rel="noflow" class="xh-social-item <?php echo $channel->svg?>" title="<?php echo $channel->title;?>"></a>
        <?php 
    }?>
</div>
<?php 
